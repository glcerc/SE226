...
def passGen(usernumber):
  rdwrd = random.sample(words, user_number)
  pw = ""
  for i in rdwrd:
    pw += i
  return pw

def rep_with_upper(pwd):
  l=len(pwd)
  pos=random.randint(0,l-1)
  strlist = list(pwd)
  strlist[pos]=strlist[pos].capitalize()
  lStr = ''.join([elem for elem in strlist])
  return lStr


def swap_letters(pwd):
  pstr=""
  if len(pwd)>3:
    pstr=str(pwd[-2:])+str(pwd[2:-2])+str(pwd[0:2])
  else:
    pstr=pwd
  return pstr


def search_letter(strng,key):
  return key in strng

user_number = int(input("Enter a number between 3-7(both inclusive):"))
if 3 <= user_number <= 7:
  pw=passGen(user_number)
  print("Your password is", pw)
  pw=rep_with_upper(pw)
  print(pw)
  pw=swap_letters(pw)
  if search_letter(pw,"i"):
    print("available")
  else:
    print("unavailable")
